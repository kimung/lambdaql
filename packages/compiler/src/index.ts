...
const QUERYABLE_METHODS = new Set(["filter", "select", "groupBy", "having", "orderBy", "orderByDesc"]);
...
export default function (program: ts.Program): ts.TransformerFactory<ts.SourceFile> {
  ...
  try {
    ...
    /* transpileModule ou programme factice */
  }
...
function transformExpr(node: ts.Expression, params: Set<string>, context: ts.TransformationContext): ts.Expression {
  ...
  if (ts.isStringLiteral(node)) return obj("ConstantExpression", [prop("value", str(node.text))]);
...
function obj(kind: string, props: ts.ObjectLiteralElementLike[]): ts.ObjectLiteralExpression {
  return ts.factory.createObjectLiteralExpression([prop("kind", str(kind)), ...props]);
}
