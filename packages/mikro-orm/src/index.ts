    ...
    case 'PropertyExpression': {
      ...
      if (chain && chain.length >= 2) {
        for (let i = 1; i < chain.length; i++) {
          ...
          if (!seen.has(key)) { seen.add(key); result.push(chain.slice(0, i)) }
        }
      }
...
export function createNamingFromMikroOrm(
  orm: MikroORM,
  entityMeta?: any,
): NamingStrategy {
...
class MikroOrmConditionBuilder {
  ...
  private likeVal(arg: Expression | undefined): string {
    if (!arg || arg.kind !== 'ConstantExpression')
      throw new Error('L\'argument LIKE doit être une constante string')
