...
describe('applyQueryable — WHERE', () => {
  it('filter simple → andWhere avec ?', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<User>('users').filter(u => u.age > 18))
    expect(qb.andWhere).toHaveBeenCalledWith('(u.age > ?)', [18])
  })

  it('deux filtres → deux appels andWhere', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<User>('users').filter(u => u.age > 18).filter(u => u.active))
  ...
  it('IS NULL', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<User>('users').filter(u => u.createdAt === null))
    expect(qb.andWhere).toHaveBeenCalledWith('u.createdAt IS NULL', [])
  })

  it('NOT unaire', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<User>('users').filter(u => !u.active))
    expect(qb.andWhere).toHaveBeenCalledWith('NOT (u.active)', [])
  })

  it('AND dans un filtre', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<User>('users').filter(u => u.age >= 18 && u.active))
    expect(qb.andWhere).toHaveBeenCalledWith('((u.age >= ?) AND u.active)', [18])
  })

  it('LIKE (includes)', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<User>('users').filter(u => u.email.includes('gmail')))
    expect(qb.andWhere).toHaveBeenCalledWith('u.email LIKE ?', ['%gmail%'])
  })

  it('IN ([].includes)', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<User>('users').filter((u: any) => [1, 2, 3].includes(u.id)))
...
describe('applyQueryable — coalesce (??)', () => {
  it('?? → COALESCE avec ?', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<User>('users').filter((u: any) => (u.name ?? 'anon') === 'x'))
...
describe('applyQueryable — sous-requêtes', () => {
  ...
  it('whereIn → IN (SELECT ...) avec ?', () => {
    ...
    applyQueryable(qb, from<User>('users').whereIn(u => u.id, inner))
    expect(qb.andWhere).toHaveBeenCalledWith(
      expect.stringContaining('u.id IN'),
  ...
  it('whereNotIn → NOT IN (SELECT ...)', () => {
    ...
    applyQueryable(qb, from<User>('users').whereNotIn(u => u.id, inner))
  ...
  it('WHERE + sous-requête combinés → deux appels andWhere', () => {
    const qb    = mockQb('u')
    const inner = from<Order>('orders').filter((o: any) => o.total > 0)
    applyQueryable(qb, from<User>('users').filter(u => u.active).whereExists(inner))
    expect(qb.andWhere).toHaveBeenCalledTimes(2)
  })
})
...
describe('applyQueryable — navigation', () => {
  it('u.books.published → books.published (convention)', () => {
    const qb = mockQb('u')
    applyQueryable(qb, from<UserWithNav>('users').filter(u => u.books.published))
...
describe('applyQueryable — auto-join via em', () => {
  ...
  it('leftJoin avant andWhere', () => {
    ...
    qb.leftJoin.mockImplementation((...args: any[]) => { callOrder.push('leftJoin'); return qb })
    qb.andWhere.mockImplementation((...args: any[]) => { callOrder.push('andWhere'); return qb })
