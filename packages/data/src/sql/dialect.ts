...
function standardFn(name: SqlFnName, args: string[]): string {
  switch (name) {
    ...
    case 'year':      return `EXTRACT(YEAR FROM ${args[0]})`
    case 'month':     return `EXTRACT(MONTH FROM ${args[0]})`
    case 'day':       return `EXTRACT(DAY FROM ${args[0]})`
    case 'dayofweek': return `EXTRACT(DOW FROM ${args[0]})`
    case 'hour':      return `EXTRACT(HOUR FROM ${args[0]})`
    case 'minute':    return `EXTRACT(MINUTE FROM ${args[0]})`
    case 'second':    return `EXTRACT(SECOND FROM ${args[0]})`
...
export const postgres: Dialect = {
  ...
  placeholder: (i) => `$${i}`,
  reindex: (sql, offset) =>
    offset === 0 ? sql : sql.replace(/\$(\d+)/g, (_, n) => `$${parseInt(n, 10) + offset}`),
  limitOffset: (limit, skip) =>
    `${limit != null ? ` LIMIT ${limit}` : ''}${skip != null ? ` OFFSET ${skip}` : ''}`,
  fn: standardFn,
...
export const sqlite: Dialect = {
  ...
  fn(name, args) {
    ...
    switch (name) {
      case 'year':      return `CAST(strftime('%Y', ${args[0]}) AS INTEGER)`
      case 'month':     return `CAST(strftime('%m', ${args[0]}) AS INTEGER)`
      case 'day':       return `CAST(strftime('%d', ${args[0]}) AS INTEGER)`
      case 'dayofweek': return `CAST(strftime('%w', ${args[0]}) AS INTEGER)`
      case 'hour':      return `CAST(strftime('%H', ${args[0]}) AS INTEGER)`
      case 'minute':    return `CAST(strftime('%M', ${args[0]}) AS INTEGER)`
      case 'second':    return `CAST(strftime('%S', ${args[0]}) AS INTEGER)`
      default:          return standardFn(name, args)
    }
  },
  quoteIdent: (id) => `"${id.replace(/"/g, '""')}"`,
}
