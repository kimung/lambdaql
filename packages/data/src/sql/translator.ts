...
export class SqlTranslator {
  ...
  translateSelect(expr: SelectExpression): SqlResult {
    ...
    const joins = expr.joins.length ? " " + expr.joins.map((j, i) => this.join(j, i + 1)).join(" ") : "";
    const whereParts = [
    ...
    const group = expr.groups.length ? " GROUP BY " + expr.groups.map((l) => this.lambdaBody(l)).join(", ") : "";
    const havingParts = [
  ...
  translateInsert(expr: InsertExpression): SqlResult {
    ...
    const values = expr.fields.map((f) => this.addParam((f.assignment as ConstantExpression).value)).join(", ");
    return {
  ...
  translateUpdate(expr: UpdateExpression): SqlResult {
    ...
    const set = expr.fields
      ...
      .join(", ");
    const where = expr.where ? ` WHERE ${this.dmlCondition(expr.where)}` : "";
    return {
  ...
  private columns(expr: SelectExpression): string {
    ...
    if (body.kind !== "ObjectLiteralExpression") return this.expr(body, aliases);
      ...
      case "MethodExpression": {
        ...
        if (
          ...
          if (AGG.has(innerFn.method)) {
            ...
            const col =
              ctxNode.kind === "NameExpression" && aliases.has((ctxNode as NameExpression).name)
                ? "*"
        ...
        if (m.context.kind === "NameExpression" && (m.context as NameExpression).name === "Math") {
          ...
          switch (m.method) {
            case "floor":
              return this.dialect.fn("floor", [arg]);
            case "ceil":
              return this.dialect.fn("ceil", [arg]);
            case "round":
              return this.dialect.fn("round", [arg]);
            case "abs":
              return this.dialect.fn("abs", [arg]);
          }
        }
        ...
        switch (m.method) {
          ...
          case "getFullYear":
            return this.dialect.fn("year", [ctx]);
          case "getMonth":
            return this.dialect.fn("month", [ctx]);
          case "getDate":
            return this.dialect.fn("day", [ctx]);
          case "getDay":
            return this.dialect.fn("dayofweek", [ctx]);
          case "getHours":
            return this.dialect.fn("hour", [ctx]);
          case "getMinutes":
            return this.dialect.fn("minute", [ctx]);
          case "getSeconds":
            return this.dialect.fn("second", [ctx]);
          default:
  ...
  private subqueryCond(cond: SubqueryCondition): string {
    ...
    if (cond.op === "EXISTS" || cond.op === "NOT EXISTS") return `${cond.op} (${inner})`;
  ...
  private buildWithClause(ctes: readonly CteExpression[]): string {
    ...
    const parts = ctes.map((cte) => {
      const child = new SqlTranslator(this.naming, this.dialect);
      const result =
        cte.query.kind === "UnionExpression"
  ...
  private buildOver(spec: ObjectLiteralExpression, aliases: Map<string, string>): string {
    ...
    for (const field of spec.fields as FieldExpression[]) {
      if (field.name === "partitionBy") {
        partitionParts =
          field.assignment.kind === "ArrayLiteralExpression"
