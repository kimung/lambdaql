...
export class Queryable<T> {
  ...
  private get _select(): SelectExpression {
    if (this._expr.kind !== "SelectExpression") throw new Error("Cannot chain query operations after union()");
  ...
  whereIn(selector: FnOrExpr<T>, inner: Queryable<any>): Queryable<T> {
    ...
    return new Queryable(
      s.patch({ subqueries: [...s.subqueries, new SubqueryCondition("IN", resolve(selector), inner._expr)] }),
      this._ctx,
    );
  }
  ...
  whereNotIn(selector: FnOrExpr<T>, inner: Queryable<any>): Queryable<T> {
    const s = this._select;
    return new Queryable(
      s.patch({ subqueries: [...s.subqueries, new SubqueryCondition("NOT IN", resolve(selector), inner._expr)] }),
      this._ctx,
    );
  }
  ...
  whereExists(inner: Queryable<any>): Queryable<T> {
    const s = this._select;
    return new Queryable(
      s.patch({ subqueries: [...s.subqueries, new SubqueryCondition("EXISTS", undefined, inner._expr)] }),
      this._ctx,
    );
  }
  ...
  whereNotExists(inner: Queryable<any>): Queryable<T> {
    const s = this._select;
    return new Queryable(
      s.patch({ subqueries: [...s.subqueries, new SubqueryCondition("NOT EXISTS", undefined, inner._expr)] }),
      this._ctx,
    );
  }
  ...
  toSql(options?: { naming?: NamingStrategy; dialect?: Dialect }): SqlResult {
    const t = new SqlTranslator(options?.naming ?? this._ctx?.naming, options?.dialect ?? this._ctx?.executor?.dialect);
    if (this._expr.kind === "UnionExpression") return t.translateUnion(this._expr as UnionExpression);
  ...
  async count(): Promise<number> {
    ...
    const base: Queryable<T> =
      this._expr.kind === "SelectExpression"
...
export function updateIn<T extends object>(
  ...
  return new SqlTranslator(options?.naming, options?.dialect).translateUpdate(
    new UpdateExpression(source, fields, whereExpr),
  );
}
