...
import {
  LambdaExpression, NameExpression, BinaryExpression, ConstantExpression,
  PropertyExpression, ObjectLiteralExpression, FieldExpression,
...
describe('filter() : runtime parse vs AST précompilé', () => {
  bench('runtime  — fn.toString() → Lexer → Parser → SqlTranslator', () => {
    from<Row>('user').filter(u => u.age > 18).toSql()
  })
...
describe('select() : runtime parse vs AST précompilé', () => {
  bench('runtime  — fn.toString() → Lexer → Parser → SqlTranslator', () => {
    from<Row>('user').select(u => ({ id: u.id, name: u.name, age: u.age })).toSql()
  })
