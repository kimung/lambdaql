...
describe("CTE — WITH basique", () => {
  it("CTE simple référencée comme source principale", () => {
    ...
    const { sql, params } = from<User>("adultes").withCte("adultes", adultes).toSql({ dialect: postgres });
  ...
  it("mysql : CTE avec placeholders ?", () => {
    const adultes = from<User>("user").filter((u) => u.age >= 18);
    const { sql, params } = from<User>("adultes").withCte("adultes", adultes).toSql({ dialect: mysql });
...
describe("CTE — WITH RECURSIVE", () => {
  it("WITH RECURSIVE quand recursive: true", () => {
    ...
    const { sql } = from<User>("org").withCte("org", baseCase, { recursive: true }).toSql({ dialect: postgres });
  ...
  it("WITH (non récursif) quand aucun recursive: true", () => {
    const adultes = from<User>("user").filter((u) => u.age >= 18);
    const { sql } = from<User>("adultes").withCte("adultes", adultes).toSql({ dialect: postgres });
...
describe("CTE — intégration avec autres clauses", () => {
  ...
  it("CTE dont la query est un UNION", () => {
    ...
    const { sql, params } = from<User>("mineurs").withCte("mineurs", q1.union(q2)).toSql({ dialect: postgres });
