...
export class Parser {
  ...
  consume(expectedKey: string): Token {
    ...
    if (token.key !== expectedKey) throw new Error(`Expected "${expectedKey}" but got "${token.key}"${token.loc()}`);
    return token;
  }
  ...
  expression(precedence = 0): Expression {
    ...
    if (!prefix) throw new Error(`Unexpected token "${token.key}"${token.loc()}`);
