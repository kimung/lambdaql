...
describe('toSql visitor', () => {
  ...
  it('includes() escapes wildcards', () => expect(sql('u => u.email.includes("a%_b")')).toBe("email LIKE '%a\\%\\_b%'"))
  it('ternary → CASE WHEN',  () => expect(sql('u => u.age >= 18 ? "adult" : "minor"')).toBe("CASE WHEN (age >= 18) THEN 'adult' ELSE 'minor' END"))
