...
console.log(
  "Engineering:",
  engineers.map((e) => `${e.name} — $${e.salary}`),
);

// 2. Nombre d'employés de plus de 27 ans
const seniorCount = await q
  .from<Employee>("employee")
  .filter((e) => e.age > 27)
  .count();
...
const withDept = await q
  .from<Employee & { deptName: string }>("employee")
...
const { sql: updSql, params: updParams } = updateIn("employee", { salary: 100000 }, (e: any) => e.id === 1, dmlOpts);
db.prepare(updSql).run(updParams);

// 7. Suppression DML (< 26 ans)
const { sql: delSql, params: delParams } = deleteFrom("employee", (e: any) => e.age < 26, dmlOpts);
